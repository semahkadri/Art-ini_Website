    ...
    && isset($_POST['name']) && isset($_POST['re_password'])&& isset($_POST['image'])) 
{
	...
	if (empty($id)) 
	{
		header("Location: signup.php?error=Identifiant est obligatoire");
	    exit();
	}
	...
	{
        header("Location: signup.php?error=Mot de passe obligatoire");
	    exit();
	...
	{
        header("Location: signup.php?error=Date de naissance obligatoire");
	    exit();
	}
	
    else if (empty($maile)) 
	{
		header("Location: signup.php?error=Adresse e-mail obligatoire");
	    exit();
	}
	
    else if(empty($name))
	{
        header("Location: signup.php?error=Nom & prénom sont obligatoire");
	    exit();
	}
	...
	{
	header("Location: signup.php?error=Mot de passe court");
	exit();

	}
	else if(strpos($maile,"@")==false || strpos($maile,".")==false)
	{
		header("Location: signup.php?error=Adresse e-mail non valide");
	    exit();
	}
	else if(empty($captcha))
	{
		header("Location: signup.php?error=Captcha obligatoire");
	    exit();
	}
	else if ($captcha !== $_SESSION['captcha_text'])
	{
		header("Location: signup.php?error=Captcha incorrecte");
	    exit();
	}
